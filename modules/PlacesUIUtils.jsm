...
var PlacesUIUtils = {
  ...
  function PUIU_showBookmarkDialog(aInfo, aParentWindow, aMinimalUI) {
    ...
    let dialogURL = aMinimalUI ?
                    "chrome://wat/content/places/bookmarkProperties2.xul" :
                    "chrome://wat/content/places/bookmarkProperties.xul";
  ...
  _openTabset: function PUIU__openTabset(aItemsToOpen, aEvent, aWindow) {
    ...
    var browserWindow =
      aWindow.document.documentElement.getAttribute("windowtype") == "mail:3pane" ?
    ...
    var loadInBackground = where == "background" ? true : false;
    //var replaceCurrentTab = where == "tab" ? false : true;
    //browserWindow.gBrowser.loadTabs(urls, loadInBackground, replaceCurrentTab);
    browserWindow.WAT.openTab(urls.shift(), loadInBackground);
    for (let i = 0; i < urls.length; i++) {
      browserWindow.WAT.openTab(urls[i], false);
    }
  },
  ...
  privateBrowsing: {
    autoStarted: false,
    lastChangedByCommandLine: false,
    privateBrowsingEnabled: false,
    removeDataFromDomain: function dummy() {},
  }
};
...
XPCOMUtils.defineLazyGetter(this, "bundle", function() {
  const PLACES_STRING_BUNDLE_URI =
    "chrome://wat/locale/places/places.properties";
